...
function Page() {
  ...
  return <SiteLayout>
    ...
    <article className="mx-auto max-w-6xl px-5 py-14 lg:px-8 lg:py-20">
      ...
      {related.length ? <section className="mt-16 border-t border-border pt-12"><p className="eyebrow">Autres projets</p><div className="mt-6 grid gap-6 md:grid-cols-3">{related.map((p) => <a key={p.id} href={`/projets/${p.slug}`} className="overflow-hidden rounded-2xl border border-border bg-card hover:shadow-elevated">{p.cover_image_url || p.image_url ? <MediaPreview url={p.cover_image_url || p.image_url || ""} alt={p.title} className="h-40 w-full object-cover" /> : null}<div className="p-5"><h2 className="text-lg">{p.title}</h2><p className="mt-2 text-sm text-muted-foreground">{p.summary}</p></div></a>)}</div></section> : null}
    </article>
  </SiteLayout>;
}